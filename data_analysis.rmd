...
sta_Innerausserorts_year <- df_escooter %>% count(`Inner-/ausserorts`,year)
sta_Innerausserorts_year$year <- factor(sta_Innerausserorts_year$year)
ggplot(data = sta_Innerausserorts_year, 
       aes(x = `Inner-/ausserorts`, y = n, group = year)) +
  geom_line(aes(colour = year)) +
  geom_point()

ggplot(data = sta_Innerausserorts_year, aes(x = `Inner-/ausserorts`, y = n, fill = year)) + 
    geom_bar(position="stack", stat="identity") +
    scale_fill_brewer(palette = "Set2")
...
colnames(df_escooter)[20] <- "Hochstgeschwindigkeit"
colnames(df_escooter)[21] <- "Strassenbeleuchtung"
